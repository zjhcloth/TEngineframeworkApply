...
public static class SettingsUtils
{
    ...
    public static string GetUpdateDataUrl()
    {
        ...
        url = $"{FrameworkGlobalSettings.WindowsUpdateDataUrl}/Version{FrameworkGlobalSettings.ScriptVersion}/update_win.json";
#elif UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX
        //url = FrameworkGlobalSettings.MacOSUpdateDataUrl;
        url = $"{FrameworkGlobalSettings.MacOSUpdateDataUrl}/Version{FrameworkGlobalSettings.ScriptVersion}/update_osx.json";
#elif UNITY_IOS
        //url = FrameworkGlobalSettings.IOSUpdateDataUrl;
        url = $"{FrameworkGlobalSettings.IOSUpdateDataUrl}/Version{FrameworkGlobalSettings.ScriptVersion}/update_ios.json";
#elif UNITY_ANDROID
        //url = FrameworkGlobalSettings.AndroidUpdateDataUrl;
        url = $"{FrameworkGlobalSettings.AndroidUpdateDataUrl}/Version{FrameworkGlobalSettings.ScriptVersion}/update_android.json";
#elif UNITY_WEBGL
        //url = FrameworkGlobalSettings.WebGLUpdateDataUrl;
        url = $"{FrameworkGlobalSettings.WebGLUpdateDataUrl}/Version{FrameworkGlobalSettings.ScriptVersion}/update_webgl.json";
#endif
        return url;
    }
    ...
    {
        get{
            int gm = GameModule.Setting.GetInt("White", 0);
            if (gm == 1) return true;
            return false;
        }
        set{
            if (value)
            {
                GameModule.Setting.SetInt("White", 1);
            }
        }
    }
    ...
    public static string GetResDownLoadPath(string fileName = "")
    {
        ...
        string resVersion = White ? UpdateData.CurrentResVersionWhite : UpdateData.CurrentResVersion;//"v2.0";//
        Debug.Log($"GetResDownLoadPath:{Path.Combine(CompleteDownLoadPath, GetPlatformName(), resVersion, fileName).Replace("\\", "/")}");
        return Path.Combine(CompleteDownLoadPath, GetPlatformName(), resVersion, fileName).Replace("\\", "/");
